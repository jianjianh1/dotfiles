...
Source: [`vimrc`](../editor/vimrc) — plugin-free, built-in features only.

The vimrc sources `~/.dotfiles-generated/vimrc.compat` for version-sensitive settings (clipboard, listchars, Neovim detection). For the Neovim config, see [neovim.md](neovim.md).
...
Generated by `install.sh` based on the detected Vim version. May include:
