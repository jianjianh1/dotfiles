...
if-shell '[ "$(~/.local/bin/detect-theme 2>/dev/null)" = "light" ]' \
  'source-file ~/.server-configs-generated/tmux-theme-light.conf' \
  'source-file ~/.server-configs-generated/tmux-theme-dark.conf'
EOF
...
_render_tmux_palette() {
    ...
    {
        echo "# Generated by setup.sh. Re-run setup.sh after changing tmux versions."
...
# Render an SSH alias that routes Remote-SSH connections through a CHPC
# login host onto an auto-allocated compute node. The alias is templated
# per-host because it embeds $USER and the absolute path to this checked-out
# repo (ProxyCommand cannot rely on PATH lookup or ~ expansion across all
# clients). The checked-in sshconfig Includes this file; SSH silently
# ignores the Include when the generated file isn't present yet.
#
# Hosts defined:
#   notchpeak-compute    → notchpeak.chpc.utah.edu → auto-alloc compute node
#
# Env override:
#   VSCODE_COMPUTE_LOGIN_HOST  login host to proxy through (default:
#                              notchpeak.chpc.utah.edu)
render_ssh_compute_alias() {
    local dst="$GENERATED_DIR/sshconfig.compute"
    local login_host="${VSCODE_COMPUTE_LOGIN_HOST:-notchpeak.chpc.utah.edu}"
    local proxy="$DIR/bin/vscode-compute-ssh-proxy.sh"

    cat > "$dst" <<EOF
# Generated by render_ssh_compute_alias() in setup.sh. Edit setup.sh, not
# this file. Included from sshconfig.
#
# Route VS Code Remote-SSH (and plain ssh) through a CHPC login host onto an
# auto-allocated "vscode-ssh" compute node. Connecting to this alias
# transparently creates/reuses an sbatch allocation and lands the session on
# the compute node — bypassing the login-node Arbiter cap that stalls
# cpptools workspace parsing.
Host notchpeak-compute
    HostName ${login_host}
    User ${USER}
    ProxyCommand ${proxy} ${login_host} ${USER}
...
render_vscode_machine_settings() {
    ...
    # Note: we deliberately do NOT branch on is_chpc_login_node here. On
    # CHPC, $HOME (and therefore ~/.vscode-server/data/Machine/settings.json)
    # is NFS-shared across login and compute nodes — a guard written on a
    # login node would disable IntelliSense on the compute node too. The
    # routing fix (Remote-SSH alias notchpeak-compute → auto-allocated
    # compute node) is what prevents the stall; bashrc_exports prints a
    # banner if you land on a login node interactively, and vscode-doctor
...
install_tree_sitter_cargo() {
    ...
    if ! command -v cargo &>/dev/null; then
        ...
        echo "  Install rustup (https://rustup.rs) and re-run setup.sh, or install tree-sitter manually."
        return 1
    fi
    if [ -n "$pin" ]; then
