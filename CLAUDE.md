...
- **Topical layout**: configs grouped under `shell/`, `editor/`, `tmux/`, `git/`, `ai/` (with `ai/skills/<name>/SKILL.md` for Claude Code skills), `chpc/` (the Notchpeak HPC agent guide deployed to `~/CLAUDE.md`, CHPC only). Operational scripts under `scripts/`. Shared shell library under `lib/`. The three top-level scripts (`install.sh`, `deploy.sh`, `uninstall.sh`) plus docs are the only flat files. No `src/` or runtime-tests tree — regression tests live at `scripts/test_regressions.sh`.
...
~/CLAUDE.md  (symlink → chpc/CLAUDE.md; CHPC only, gated by is_chpc())
...
- Symlinks `chpc/CLAUDE.md` → `~/CLAUDE.md` (the Notchpeak HPC agent guide every CHPC agent loads), via `link_chpc_agent_guide`. This is the only repo file deployed to `~/CLAUDE.md`; edit `chpc/CLAUDE.md`, never `~/CLAUDE.md` directly. Skipped off-CHPC.
