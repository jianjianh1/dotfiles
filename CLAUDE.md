...
- **`setup.sh`** — run locally to symlink configs into `$HOME` and install CLI tools (glow, nvim, node, uv, Claude Code, Codex, VS Code `code` tunnel CLI + openvscode-server, etc.). Also pre-installs VS Code extensions (`anthropic.claude-code`, `openai.chatgpt`, `ms-vscode.cpptools`, `ms-python.python`) into `~/.vscode-server/extensions/` so Remote-SSH / `code tunnel` sessions land ready. The shell aliases also define `vscode-tunnel` and `prefetch-vscode-server` helpers — see `bashrc_aliases`.
