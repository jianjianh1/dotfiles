...
- **Topical layout**: configs grouped under `shell/`, `editor/`, `tmux/`, `git/`, `ssh/`, `ai/` (with `ai/skills/<name>/SKILL.md` for Claude Code skills). Operational scripts under `scripts/`. Shared shell library under `lib/`. The three top-level scripts (`install.sh`, `deploy.sh`, `uninstall.sh`) plus docs are the only flat files. No `src/` or runtime-tests tree — regression tests live at `scripts/test_regressions.sh`.
...
~/.ssh/config
  └── Include line → ssh/sshconfig  (appended; not a symlink)
