...
- **Topical layout**: configs grouped under `shell/`, `editor/`, `tmux/`, `git/`, `ssh/`, `ai/`. Operational scripts under `scripts/`. Shared shell library under `lib/`. The three top-level scripts (`install.sh`, `deploy.sh`, `uninstall.sh`) plus docs are the only flat files. No `src/` or runtime-tests tree — regression tests live at `scripts/test_regressions.sh`.
