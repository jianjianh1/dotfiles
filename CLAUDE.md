...
- **`setup.sh`** — run locally to symlink configs into `$HOME` and install CLI tools (glow, nvim, node, uv, Claude Code, Codex, VS Code `code` tunnel CLI + openvscode-server, etc.). The shell aliases also define `vscode-tunnel` and `prefetch-vscode-server` helpers — see `bashrc_aliases`.
